...
/* ─────────────────────────────────────────────────────────────────────────────
   NOTE: install react-firebase-hooks if not already:
     npm install react-firebase-hooks
   It gives us useAuthState which handles auth loading state cleanly.
───────────────────────────────────────────────────────────────────────────── */
...
/* ─────────────────────────────────────────────────────────────────────────────
   SMALL COMPONENTS (reusable buttons)
───────────────────────────────────────────────────────────────────────────── */
...
function CatBtn({ active, onClick, children }: { active: boolean; onClick: () => void; children: React.ReactNode }) {
  return (
    <button onClick={onClick} style={{
      ...
      color: active ? "#F5ECD7" : "#C8A98A", fontFamily:"'Barlow', sans-serif",
      fontSize:12, fontWeight:500, borderRadius:4, cursor:"pointer", textAlign:"left", marginBottom:2,
    }}>{children}</button>
  );
}
...
function BottomTabBar({ view, setView, activeCount }: {
  view: string; setView: (v: string) => void; activeCount: number;
}) {
  const tabs = [
  ...
  return (
    <div style={{
      ...
      background:"#3B1F0E", display:"flex",
      borderTop:"1px solid #5a3020", zIndex:100,
        ...
        <button key={t.id} onClick={() => setView(t.id)} style={{
          ...
          {t.badge ? (
            <span style={{
              ...
              background:"#C0622A", color:"#fff", borderRadius:10,
    ...
    <>
      ...
      <div style={{
        ...
        <div style={{ padding:"10px 16px 0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div style={{ fontFamily:"'Barlow Condensed', sans-serif", fontWeight:800, fontSize:20, color:"#3B1F0E" }}>
            ORDER
          </div>
          <button onClick={onClose} style={{
            background:"none", border:"none", fontSize:20, cursor:"pointer", color:"#8A6040",
          }}>✕</button>
        </div>
...
export default function App() {
  ...
  const addToCart = (item: CartItem["id"] extends string ? any : never, sizeLabel: string, price: number) => {
    setCart(prev => {
  ...
  /* ─── Cart content (shared between sidebar panel and mobile drawer) ─────── */
  const CartContent = () => (
    <>
      ...
      <div style={S.cartFooter}>
        ...
        <div style={S.footerInputs}>
          <input style={{ flex:1, padding:"7px 8px", border:"1px solid #DDD0C0", borderRadius:6, fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#3B1F0E", background:"#FAF6EF", outline:"none" }} placeholder="Discount ₱" value={discount} onChange={e => setDiscount(e.target.value)} />
          <input style={{ flex:1, padding:"7px 8px", border:"1px solid #DDD0C0", borderRadius:6, fontFamily:"'Barlow', sans-serif", fontSize:12, color:"#3B1F0E", background:"#FAF6EF", outline:"none" }} placeholder="Cash ₱" value={cash} onChange={e => setCash(e.target.value)} />
        </div>
        {cash && Number(cash) >= total && (
          <div style={S.changeRow}><span>Change</span><span>₱{change}</span></div>
        )}
        {cash && Number(cash) < total && (
          <div style={{ ...S.changeRow, background:"#FDECEA", color:"#C0622A" }}><span>Short by</span><span>₱{total - Number(cash)}</span></div>
        )}
  ...
  return (
    <div style={{ ...S.root, paddingBottom: isMobile ? 60 : 0 }}>
      ...
      {view === "cashier" && (
        <div style={{ flex:1, display:"flex", overflow:"hidden" }}>
          <div style={S.productArea}>
            ...
            {/* Product grid */}
            <div style={{
              ...{ flex:1, overflowY:"auto", padding:10, display:"grid", gap:8, alignContent:"start" },
      ...
      {/* ── MOBILE: bottom tab bar ── */}
      {isMobile && (
        <BottomTabBar view={view} setView={setView} activeCount={ordersCtx.activeOrders.length} />
      )}
